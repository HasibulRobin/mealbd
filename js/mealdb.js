...
const searchFood = () => {
    ...
    searchField.value = '';
    if (searchText == '') {
        alert('Search a meal that you want to eat..')
    }
    else {
...
const displaySearchResult = (meals) => {
    console.log(meals);
    const searchResult = document.getElementById('search-food');
    // clear the display when search a new food
    searchResult.textContent = '';
    // while meal is not available
    /*  if (meals.length == 0) {
         const notAvailable = document.getElementById('not-available');
         const heading = document.getElementById('heading');
         heading.innerText = 'sorry,your food is not found !'
         notAvailable.appendChild(heading)
     }
  */
    meals.forEach(meal => {
        // console.log(meal);
